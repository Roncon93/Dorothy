...
#include "generated/jsb_dorothy_auto.hpp"
...
JSBool JSB_cocos2dx_CCSprite_createWithClip(JSContext *cx, uint32_t argc, jsval *vp)
{
	jsval *argv = JS_ARGV(cx, vp);
	JSBool ok = JS_TRUE;
	do {
		if (argc == 1) {
			...
			cocos2d::CCSprite* ret = Dorothy::oClipCache::shared()->loadSprite(arg0);
			jsval jsret;
			do {
				if (ret) {
...
static bool isInitializingClass = false;
bool ccIsInitializingClass()
{
	return isInitializingClass;
}
JSBool JSB_cocos2dx_setInitializingClass(JSContext *cx, uint32_t argc, jsval *vp)
{
	jsval *argv = JS_ARGV(cx, vp);
	JSBool ok = JS_TRUE;
	do
	{
		if (argc == 1)
		{
			JSBool arg0 = JSVAL_TO_BOOLEAN(argv[0]);
			isInitializingClass = (arg0 == JS_TRUE);
			return JS_TRUE;
		}
	} while (0);
	JS_ReportError(cx, "wrong number of arguments");
...
// oModelHandlerWrapper
class oModelHandlerWrapper
{
public:
	oModelHandlerWrapper(jsval callback, JSObject* obj)
		:jsCallback(callback), jsThisObj(obj){}
	jsval jsCallback;
	JSObject* jsThisObj;
	const oModelHandlerWrapper& operator*() const
	{
		return *this;
	}
	bool operator==(const oModelHandlerWrapper& handler) const
	{
		return jsCallback == handler.jsCallback && jsThisObj == handler.jsThisObj;
	}
	bool operator!=(const oModelHandlerWrapper& handler) const
	{
		return jsCallback != handler.jsCallback || jsThisObj != handler.jsThisObj;
	...
	{
		JSContext *cx = ScriptingCore::getInstance()->getGlobalContext();
		js_proxy_t *proxy = js_get_or_create_proxy<Dorothy::oModel>(cx, model);
		jsval retval;
		if(jsCallback != JSVAL_VOID)
		{
			jsval senderVal = OBJECT_TO_JSVAL(proxy->obj);
			JS_AddValueRoot(cx, &senderVal);
			JS_CallFunctionValue(cx, jsThisObj, jsCallback, 1, &senderVal, &retval);
			JS_RemoveValueRoot(cx, &senderVal);
		}
	}
};
JSBool js_dorothy_oModel_addHandler(JSContext *cx, uint32_t argc, jsval *vp)
{
	if (argc == 2)
	{
		jsval *argv = JS_ARGV(cx, vp);
		...
		do 
		{
			...
			if (!ok) { ok = JS_TRUE; break; }
			cobj->handlers[arg0] += std::make_pair(oModelHandlerWrapper(argv[1],obj), &oModelHandlerWrapper::call);
			return JS_TRUE;
		} while (0);
	}
	JS_ReportError(cx, "Invalid number of arguments");
...
JSBool js_dorothy_oModel_removeHandler(JSContext *cx, uint32_t argc, jsval *vp)
{
	if (argc == 2)
	{
		jsval *argv = JS_ARGV(cx, vp);
...
// oSensorHandlerWrapper
class oSensorHandlerWrapper
{
public:
	oSensorHandlerWrapper(jsval callback, JSObject* obj)
		:jsCallback(callback), jsThisObj(obj){}
	jsval jsCallback;
	JSObject* jsThisObj;
	const oSensorHandlerWrapper& operator*() const
	{
		return *this;
	}
	bool operator==(const oSensorHandlerWrapper& handler) const
	{
		return jsCallback == handler.jsCallback && jsThisObj == handler.jsThisObj;
	}
	bool operator!=(const oSensorHandlerWrapper& handler) const
	...
	{
		JSContext *cx = ScriptingCore::getInstance()->getGlobalContext();
		js_proxy_t *proxy0 = js_get_or_create_proxy<Dorothy::oSensor>(cx, sensor);
		js_proxy_t *proxy1 = js_get_or_create_proxy<Dorothy::oBody>(cx, body);
		jsval retval;
		if(jsCallback != JSVAL_VOID)
		{
			jsval senderVals[] =
			{
				OBJECT_TO_JSVAL(proxy0->obj),
				OBJECT_TO_JSVAL(proxy1->obj)
			};
			JS_AddValueRoot(cx, senderVals);
			JS_CallFunctionValue(cx, jsThisObj, jsCallback, 2, senderVals, &retval);
			JS_RemoveValueRoot(cx, senderVals);
		}
	}
};
JSBool js_dorothy_oSensor_addEnterHandler(JSContext *cx, uint32_t argc, jsval *vp)
{
	if (argc == 1)
	{
		jsval *argv = JS_ARGV(cx, vp);
		...
		cobj->bodyEnter += std::make_pair(oSensorHandlerWrapper(argv[0],obj), &oSensorHandlerWrapper::call);
		return JS_TRUE;
	}
	JS_ReportError(cx, "Invalid number of arguments");
	return JS_FALSE;
}
JSBool js_dorothy_oSensor_removeEnterHandler(JSContext *cx, uint32_t argc, jsval *vp)
{
	if (argc == 1)
	{
		jsval *argv = JS_ARGV(cx, vp);
...
JSBool js_dorothy_oSensor_addLeaveHandler(JSContext *cx, uint32_t argc, jsval *vp)
{
	if (argc == 1)
	{
		jsval *argv = JS_ARGV(cx, vp);
		...
		js_proxy_t *proxy; JS_GET_NATIVE_PROXY(proxy, obj);
		Dorothy::oSensor* cobj = (Dorothy::oSensor *)(proxy ? proxy->ptr : NULL);
		cobj->bodyLeave += std::make_pair(oSensorHandlerWrapper(argv[0],obj), &oSensorHandlerWrapper::call);
		return JS_TRUE;
	}
	JS_ReportError(cx, "Invalid number of arguments");
	return JS_FALSE;
}
JSBool js_dorothy_oSensor_removeLeaveHandler(JSContext *cx, uint32_t argc, jsval *vp)
{
	if (argc == 1)
	{
		jsval *argv = JS_ARGV(cx, vp);
...
JSBool js_dorothy_oSensor_sensedBodies(JSContext *cx, uint32_t argc, jsval *vp)
{
	JSObject *obj = JS_THIS_OBJECT(cx, vp);
	js_proxy_t *proxy; JS_GET_NATIVE_PROXY(proxy, obj);
	Dorothy::oSensor* cobj = (Dorothy::oSensor*)(proxy ? proxy->ptr : NULL);
	...
	if (argc == 0) {
		jsval jsret;
		jsret = refvector_of_bodies_to_jsval(cx, cobj->getSensedBodies());
		JS_SET_RVAL(cx, vp, jsret);
		return JS_TRUE;
	}
	JS_ReportError(cx, "wrong number of arguments: %d, was expecting %d", argc, 0);
...
JSBool js_dorothy_oWorld_query(JSContext *cx, uint32_t argc, jsval *vp)
{
	if (argc == 2)
	{
		jsval *argv = JS_ARGV(cx, vp);
		...
		do 
		{
			...
			jsval arg1 = argv[1];
			JSB_PRECONDITION2(ok, cx, JS_FALSE, "Error processing arguments");
			cobj->query(arg0, [&](oBody* body)
			{
				proxy = js_get_or_create_proxy<Dorothy::oBody>(cx, body);
				jsval retval;
				jsval senderVal = OBJECT_TO_JSVAL(proxy->obj);
				JS_AddValueRoot(cx, &senderVal);
				JS_CallFunctionValue(cx, obj, arg1, 1, &senderVal, &retval);
				JS_RemoveValueRoot(cx, &senderVal);
				JSBool ret = JSVAL_TO_BOOLEAN(retval);
				return (bool)ret;
			});
		} while (0);
		return JS_TRUE;
...
void register_cocos2dx_js_specifics(JSContext* cx, JSObject* global)
{
	...
	tmpObj = JSVAL_TO_OBJECT(anonEvaluate(cx, global, "(function () { return cc.Sprite; })()"));
	JS_DefineFunction(cx, tmpObj, "createWithClip", JSB_cocos2dx_CCSprite_createWithClip, 2, JSPROP_READONLY | JSPROP_PERMANENT);

	tmpObj = JSVAL_TO_OBJECT(anonEvaluate(cx, global, "(function () { return cc; })()"));
	JS_DefineFunction(cx, tmpObj, "setInitializingClass", JSB_cocos2dx_setInitializingClass, 1, JSPROP_READONLY | JSPROP_PERMANENT);

	JS_DefineFunction(cx, jsb_oModel_prototype, "addHandler", js_dorothy_oModel_addHandler, 2, JSPROP_READONLY | JSPROP_PERMANENT);
	JS_DefineFunction(cx, jsb_oModel_prototype, "removeHandler", js_dorothy_oModel_removeHandler, 2, JSPROP_READONLY | JSPROP_PERMANENT);
	
	JS_DefineFunction(cx, jsb_oSensor_prototype, "addEnterHandler", js_dorothy_oSensor_addEnterHandler, 1, JSPROP_READONLY | JSPROP_PERMANENT);
	JS_DefineFunction(cx, jsb_oSensor_prototype, "removeEnterHandler", js_dorothy_oSensor_removeEnterHandler, 1, JSPROP_READONLY | JSPROP_PERMANENT);
	JS_DefineFunction(cx, jsb_oSensor_prototype, "addLeaveHandler", js_dorothy_oSensor_addLeaveHandler, 1, JSPROP_READONLY | JSPROP_PERMANENT);
	JS_DefineFunction(cx, jsb_oSensor_prototype, "removeLeaveHandler", js_dorothy_oSensor_removeLeaveHandler, 1, JSPROP_READONLY | JSPROP_PERMANENT);
	JS_DefineFunction(cx, jsb_oSensor_prototype, "sensedBodies", js_dorothy_oSensor_sensedBodies, 0, JSPROP_READONLY | JSPROP_PERMANENT);

	JS_DefineFunction(cx, jsb_oWorld_prototype, "query", js_dorothy_oWorld_query, 2, JSPROP_READONLY | JSPROP_PERMANENT);
