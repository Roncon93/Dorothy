    ...
    if (argc >= 1) {
        ...
        if (argc == 2 && argv[1].isString()) {
            ...
            if (rootedGlobal) {
                ...
                JS_ReportError(cx, "Invalid global object: %s", (const char*)name);
                return JS_FALSE;
            }
        } else {
...
JSBool jsval_to_b2vec2(JSContext *cx, jsval v, b2Vec2* ret) {
	JSObject *tmp;
	jsval jsx, jsy;
	double x, y;
	JSBool ok = JS_ValueToObject(cx, v, &tmp) &&
...
JSBool jsval_to_b2vec2vector(JSContext* cx, jsval v, vector<b2Vec2>* ret)
{
	JSObject *jsobj;
	JSBool ok = JS_ValueToObject( cx, v, &jsobj );
	JSB_PRECONDITION2( ok, cx, JS_FALSE, "Error converting value to object");
	...
	for( uint32_t i=0; i< len;i++ ) {
		jsval valarg;
		JS_GetElement(cx, jsobj, i, &valarg);
		ok = jsval_to_b2vec2(cx, valarg, &(*ret)[i]);
		JSB_PRECONDITION2(ok, cx, JS_FALSE, "Error processing arguments");
	}
	return JS_TRUE;
}
...
jsval b2vec2_to_jsval( JSContext* cx, b2Vec2& v )
{
	JSObject *tmp = JS_NewObject(cx, NULL, NULL, NULL);
	if (!tmp) return JSVAL_NULL;
	JSBool ok = JS_DefineProperty(cx, tmp, "x", DOUBLE_TO_JSVAL(v.x), NULL, NULL, JSPROP_ENUMERATE | JSPROP_PERMANENT) &&
...
jsval refvector_of_bodies_to_jsval( JSContext* cx, const Dorothy::oRefVector<Dorothy::oBody>& bodies )
{
	JSObject *jsretArr = JS_NewArrayObject(cx, bodies.size(), NULL);
	for (int i = 0; i < (int)bodies.size(); i++)
	{
		jsval arrElement;
		js_proxy_t* jsproxy = js_get_or_create_proxy<Dorothy::oBody>(cx, bodies[i]);
		arrElement = OBJECT_TO_JSVAL(jsproxy->obj);
...
JSBool jsGetScript(JSContext* cx, unsigned argc, jsval* vp)
{
    ...
    if (argc == 1 && argv[0].isString()) {
        ...
        JSStringWrapper wrapper(str);
        JSScript* script = filename_script[(const char*)wrapper];
        if (script) {
