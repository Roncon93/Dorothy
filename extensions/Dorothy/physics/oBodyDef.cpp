...
void oBodyDef::attachPolygon( const b2Vec2& center, float width, float height, float angle, float density, float friction, float restitution )
{
...
void oBodyDef::attachPolygon( const vector<b2Vec2>& vertices, float density, float friction, float restitution )
{
	b2PolygonShape* shape = new b2PolygonShape();
...
void oBodyDef::attachPolygon( const b2Vec2 vertices[], int count, float density, float friction, float restitution )
{
	b2PolygonShape* shape = new b2PolygonShape();
	b2Vec2 vs[b2_maxPolygonVertices];
...
void oBodyDef::attachLoop( const vector<b2Vec2>& vertices, float friction, float restitution )
{
...
void oBodyDef::attachCircle( const b2Vec2& center, float radius, float density, float friction, float restitution )
{
...
{
	oBodyDef::attachCircle(b2Vec2(0,0), radius, density, friction, restitution);
}
void oBodyDef::attachChain( const vector<b2Vec2>& vertices, float friction, float restitution )
{
	b2ChainShape* shape = new b2ChainShape();
	int length = vertices.size();
	b2Vec2* vs = new b2Vec2[length];
...
{
	oBodyDef::attachPolygonSensor(tag, width, height, b2Vec2(0,0), 0);
}
void oBodyDef::attachPolygonSensor( int tag, float width, float height, const b2Vec2& center, float angle )
{
	b2PolygonShape* shape = new b2PolygonShape();
	shape->SetAsBox(oWorld::b2Val(width * 0.5f),
		oWorld::b2Val(height * 0.5f),
...
void oBodyDef::attachPolygonSensor( int tag, const vector<b2Vec2>& vertices )
{
	b2PolygonShape* shape = new b2PolygonShape();
	int length = vertices.size();
	b2Vec2 vs[b2_maxPolygonVertices];
...
void oBodyDef::attachCircleSensor( int tag, const b2Vec2& center, float radius )
{
	b2CircleShape* shape = new b2CircleShape();
	shape->m_p = oWorld::b2Val(center);
	shape->m_radius = oWorld::b2Val(radius);
...
{
	oBodyDef::attachCircleSensor(tag, b2Vec2(0,0), radius);
}
...
b2FixtureDef* oBodyDef::polygon( const b2Vec2 vertices[], int count, float density, float friction, float restitution )
{
	b2Vec2 vs[b2_maxPolygonVertices];
	for (int i = 0; i < count; i++)
	{
...
void oBodyDef::setType( uint32 type )
{
	b2BodyDef::type = (b2BodyType)type;
}

void oBodyDef::setFixedRotation( bool fixed )
{
	b2BodyDef::fixedRotation = fixed;
}

void oBodyDef::setIsBullet( bool isBullet )
{
	b2BodyDef::bullet = isBullet;
}

void oBodyDef::setGravityScale( float scale )
{
	b2BodyDef::gravityScale = scale;
}
